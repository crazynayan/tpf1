...
class NameSuccessETAW(unittest.TestCase):
    ...
    def test_multiple_names_wa0pty_match_ETAW(self) -> None:
        ...
        TD.state.setup.aaa['WA0PTY'] = bytearray([99])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(0xF0F0, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(99, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(99, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(TD.ui2214, TD.state.vm.get_byte(TD.state.regs.R7 + TD.ui2cnn))
    ...
    def test_WA0NAD_ETAW(self) -> None:
        ...
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual(0xF0F0, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(4, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(4, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertTrue(TD.state.vm.all_bits_off(TD.state.regs.R1 + TD.wa0etg, TD.wa0nad))
        self.assertEqual(7, TD.state.regs.R6)  # Call to ETK2 with R6=7 will ensure Name association is deleted

    def test_WA0CDI_ETAW(self) -> None:
        Pnr.add_names(config.AAAPNR, ['33ZAVERI'])
        TD.wa0cdi = macros['WA0AA'].evaluate('#WA0CDI')
        TD.state.setup.aaa['WA0US4'] = bytearray([TD.wa0cdi])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        TD.state.run('ETA5', aaa=True)
        self.assertEqual(0xF0F0, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(33, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(33, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(60, TD.state.regs.R6)
    ...
    def test_group_C_ETAW(self) -> None:
        ...
        self.assertEqual('$$ETAW$$.1', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(0xF1F9, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(21, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(21, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebsw01, 0x10))
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebw000 + 38, 0x80))
        self.assertEqual('C', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebw000 + 14)).decode)

    def test_group_Z_ETAW_wa0pyt_no_match(self) -> None:
        Pnr.add_names(config.AAAPNR, ['Z/25SABRE', '3SHAH'])
        TD.state.setup.aaa['WA0PTY'] = bytearray([3])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(0xF2F2, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(25, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(25, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebsw01, 0x10))
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebw000 + 38, 0x80))
        self.assertEqual('Z', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebw000 + 14)).decode)

    def test_group_C_not_at_start_wa0pty_history_no_match_ETAW(self):
        Pnr.add_names(config.AAAPNR, ['10ZAVERI', 'C/21TOURS', '1SHAH'])
        TD.state.setup.aaa['WA0PTY'] = bytearray([0xE3])  # 99 = 0x63 with bit0 on is 0xE3
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(0xF1F0, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual('C', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebw000 + 14)).decode)
        self.assertEqual(21, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(0x95, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))  # 21 = 0x15 with bit0 on is 0x95

    def test_pn2_on_group_9_C_ETAW(self):
        Pnr.add_names(config.AAAPNR, ['C/99W/TOURS', '3SHAH'])
        TD.state.setup.aaa['WA0UB1'] = bytearray([TD.wa0pn2])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual(0xF0F6, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(9, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(9, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_pn2_on_group_99_C_ETAW(self):
        ...
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual(0xF9F6, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(99, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(99, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_pn2_on_group_Z_wa0pty_history_match_ETAW(self):
        ...
        self.assertEqual(0xF9F6, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(99, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(0xE3, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))

    def test_pn2_off_group_wa0pti_ETAW(self):
        Pnr.add_names(config.AAAPNR, ['C/99W/TOURS', '3SHAH'])
        TD.state.setup.aaa['WA0UB1'] = bytearray([0x00])
        TD.state.setup.aaa['WA0PTI'] = bytearray([3])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertIn('021014', TD.state.dumps)
        self.assertEqual(0xF9F6, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(99, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(99, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(0, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))
    ...
    def test_infant_with_adults_ETAW(self):
        ...
        self.assertIn('021014', TD.state.dumps)
        self.assertEqual(0xF0F0, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(3, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(3, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(1, TD.state.vm.get_byte(TD.ebw000 + 10))
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))

    def test_infant_only_ETAW(self):
        Pnr.add_names(config.AAAPNR, ['I/3ZAVERI'])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual(0xF0F3, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(3, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(3, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(3, TD.state.vm.get_byte(TD.ebw000 + 10))
        self.assertEqual(3, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))
    ...
    def test_infant_at_start_with_less_adults_ETAW(self):
        ...
        self.assertEqual(99, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(99, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(55, TD.state.vm.get_byte(TD.ebw000 + 10))
        self.assertEqual(55, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))
    ...
    def test_infant_with_group_wa0pti_ETAW(self):
        ...
        self.assertEqual('$$ETAW$$.1', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(0xF1F4, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(24, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(24, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(3, TD.state.vm.get_byte(TD.ebw000 + 10))
        self.assertEqual(3, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))
    ...
    def test_infant_at_start_with_group_ETAW(self):
        ...
        self.assertEqual('$$ETAW$$.1', label)
        self.assertIn('021014', TD.state.dumps)
        self.assertEqual(0xF1F6, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))
        self.assertEqual(32, TD.state.vm.get_byte(TD.ebw000 + 15))
        self.assertEqual(0, TD.state.vm.get_byte(TD.ebw000 + 16))
        self.assertEqual(32, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
        self.assertEqual(7, TD.state.vm.get_byte(TD.ebw000 + 10))
        self.assertEqual(7, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))
...
class NameSuccessVarious(unittest.TestCase):
    ...
    def test_infant_group_call_from_TOQ1(self) -> None:
        Pnr.add_names(config.AAAPNR, ['I/5ZAVERI', '3ZAVERI', 'C/25TOURS', '6SHAH', 'I/2SHAH'])
        label = TD.state.run('TOQ1', aaa=True)
        self.assertEqual('$$TOQ1$$.4', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(0xF1F6, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))  # Group - Adults
        self.assertEqual(32, TD.state.vm.get_byte(TD.ebw000 + 15))  # Group + Infants
        self.assertEqual(9, TD.state.vm.get_byte(TD.ebw000 + 16))  # Adults
        self.assertEqual(7, TD.state.vm.get_byte(TD.ebw000 + 10))  # Infants
        self.assertEqual(0, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))  # Group + Infants
        self.assertEqual(0, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))  # Infants

    def test_infant_group_with_error_from_itinerary_police_not_file_rec_mode_ETA2(self) -> None:
        Pnr.add_names(config.AAAPNR, ['I/5ZAVERI', '3ZAVERI', 'C/25TOURS', '6SHAH', 'I/2SHAH'])
        TD.state.setup.aaa['WA0PTI'] = bytearray([7])  # Infants
        TD.state.setup.aaa['WA0PTY'] = bytearray([32])  # Group + Infants
        TD.state.setup.errors.add('ETC1ERR')  # Error from Basic Itinerary Police
        TD.state.setup.aaa['WA0ET3'] = bytearray([0x00])  # File Rec mode indicator is OFF
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETA2$$.1', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(0xF1F6, TD.state.vm.get_unsigned_value(TD.state.regs.R1 + TD.wa0ext, 2))  # Group - Adults
        self.assertEqual(32, TD.state.vm.get_byte(TD.ebw000 + 15))  # Group + Infants
        self.assertEqual(0, TD.state.vm.get_byte(TD.ebw000 + 16))  # Adults
        self.assertEqual(7, TD.state.vm.get_byte(TD.ebw000 + 10))  # Infants
        self.assertEqual(32, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))  # Group + Infants
        self.assertEqual(7, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pti))  # Infants

    def test_infant_group_with_error_from_itinerary_police_file_rec_mode_FRD(self) -> None:
        Pnr.add_names(config.AAAPNR, ['I/5ZAVERI', '3ZAVERI', 'C/25TOURS', '6SHAH', 'I/2SHAH'])
        TD.state.setup.aaa['WA0PTI'] = bytearray([0])  # Infants
        TD.state.setup.aaa['WA0PTY'] = bytearray([0])  # Group + Infants
        TD.state.setup.errors.add('ETC1ERR')  # Error from Basic Itinerary Police
        TD.state.setup.aaa['WA0ET3'] = bytearray([0x10])  # File Rec mode indicator is ON
        label = TD.state.run('ETA5', aaa=True)
...
class NameFailETK1(unittest.TestCase):
    ...
    def test_no_name_tty_ETK1_33(self) -> None:
        TD.state.setup.aaa['WA0ET4'] = bytearray([TD.wa0tty])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETK1$$.1', label)
        self.assertEqual(0, TD.state.vm.get_byte(TD.state.regs.R7 + TD.ui2cnn))
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebrs01, 0x60))
        self.assertEqual(33, TD.state.regs.R6)
    ...
    def test_too_many_names_tty_ETK1_33(self) -> None:
        ...
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETK1$$.1', label)
        self.assertEqual(100, TD.state.regs.R15)
        self.assertEqual(TD.ui2098, TD.state.vm.get_byte(TD.state.regs.R7 + TD.ui2cnn))
        self.assertEqual(33, TD.state.regs.R6)
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebrs01, 0x60))
    ...
    def test_multiple_groups_CC_tty_ETK1_33(self):
        ...
        self.assertEqual('$$ETK1$$.1', label)
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(TD.ui2097, TD.state.vm.get_byte(TD.state.regs.R7 + TD.ui2cnn))
        self.assertEqual(33, TD.state.regs.R6)
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebrs01, 0x60))
        self.assertEqual('C', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebw000 + 14)).decode)
    ...
    def test_multiple_groups_CZ_tty_ETK1_16(self):
        ...
        self.assertListEqual(list(), TD.state.dumps)
        self.assertEqual(TD.ui2098, TD.state.vm.get_byte(TD.state.regs.R7 + TD.ui2cnn))
        self.assertEqual(16, TD.state.regs.R6)
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebrs01, 0xC0))
        self.assertEqual('C', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebw000 + 14)).decode)
    ...
    def test_invalid_type_ETK1_16(self):
        ...
        self.assertEqual(16, TD.state.regs.R6)
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebrs01, 0xC0))
        self.assertEqual('K', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebw000 + 14)).decode)
    ...
    def test_group_Z_not_at_start_ETK1_16(self):
        ...
        self.assertEqual(16, TD.state.regs.R6)
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebrs01, 0xC0))
        self.assertEqual(0x00, TD.state.vm.get_byte(TD.ebw000 + 14))
...
class NameFailETA5(unittest.TestCase):
    ...
    def test_too_many_names_ETA5430(self) -> None:
        ...
        self.assertEqual("'MAXIMUM NUMBER OF NAMES PER PNR IS 99 - CREATE NEW PNR'", TD.state.message)
        self.assertEqual(100, TD.state.regs.R15)
        self.assertEqual(TD.ui2098, TD.state.vm.get_byte(TD.state.regs.R7 + TD.ui2cnn))
...
class NameFailUIO1(unittest.TestCase):
    ...
    def test_group_overbooking_UIO1(self):
        ...
        self.assertTrue(TD.state.vm.all_bits_on(TD.state.regs.R1 + TD.wa0et4, 0x20))
        ...
        self.assertEqual(ui2inc_bytes, TD.state.vm.get_bytes(TD.state.regs.R7 + ui2inc, 3))
        self.assertTrue(TD.state.vm.all_bits_off(TD.state.regs.R1 + TD.wa0et5, 0x02))
        self.assertTrue(TD.state.vm.all_bits_on(TD.state.regs.R1 + TD.wa0et5, TD.wa0any))
        self.assertEqual(TD.ui2214, TD.state.vm.get_byte(TD.state.regs.R7 + TD.ui2cnn))
        self.assertTrue(TD.state.vm.all_bits_on(TD.ebrs01, 0x60))
...
class NameException(unittest.TestCase):
    ...
    def test_invalid_group_at_start_Exception(self):
        ...
        self.assertRaises(PackExecutionError, TD.state.run, 'ETA5', True)
        self.assertEqual('Z', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebw000 + 14)).decode)
...
class Companion(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        Pnr.add_names(config.AAAPNR, ['1ZAVERI'])
        Tpfdf.init_db()
        Tpfdf.add(data=TD.tr1gaa, ref_name='TR1GAA', key='40')
        TD.state.init_run()
        TD.state.setup.aaa['WA0ET6'] = bytearray([TD.wa0hfx])
    ...
    def test_fqtv_itin_match_award_not_exp_key_ETK2(self) -> None:
        ...
        self.assertEqual(0x60, TD.state.vm.get_byte(TD.ebrs01))
        self.assertEqual(116, TD.state.regs.R6)

    def test_fqtv_no_match_award_not_exp_key_ETK2(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld)
        Pnr.add_fqtv('DGHWCL', TD.fqtv_exp_key)
        Pnr.add_itin('DGHWCL', TD.itin_2811_2812)
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('ETK20100.1', label)
    ...
    def test_date_error_ETK2(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld_date_error)
        Pnr.add_fqtv('DGHWCL', TD.fqtv_gld)
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('ETK20100.1', label)
        self.assertNotEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
    ...
    def test_fqtv_itin_match_no_award_exp_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2811_exp)
        Pnr.add_fqtv('DGHWCL', TD.fqtv_exp_key)
        Pnr.add_itin('DGHWCL', TD.itin_2811_2812)
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
    ...
    def test_fqtv_itin_match_award_exp_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_exp)
        Pnr.add_fqtv('DGHWCL', TD.fqtv_exp_key)
        Pnr.add_itin('DGHWCL', TD.itin_2811_2812)
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
    ...
    def test_fqtv_itin_match_award_key_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_key)
        Pnr.add_fqtv('DGHWCL', TD.fqtv_exp_key)
        Pnr.add_itin('DGHWCL', TD.itin_2811_2812)
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
    ...
    def test_tr1gaa_error_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld)
        TD.state.setup.errors.add('ETA92100.1')
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_dbifb_error_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld)
        TD.state.setup.errors.add('ETA92300.1')
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_pnrcc_error_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld)
        TD.state.setup.errors.add('ETA92300.10')
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_prp1_error_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld)
        TD.state.setup.errors.add('PRP1ERR')
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_fqtv_error_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld)
        TD.state.setup.errors.add('ETA92400.1')
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_itin_error_ETAW(self) -> None:
        Pnr.add_hfax(config.AAAPNR, TD.hfax_2812_gld)
        Pnr.add_fqtv('DGHWCL', TD.fqtv_gld)
        TD.state.setup.errors.add('ETA92500.1')
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
    ...
    def test_fqtv_itin_match_award_error_ETAW(self) -> None:
        ...
        Pnr.add_fqtv('DGHWCL', TD.fqtv_gld)
        Pnr.add_itin('DGHWCL', TD.itin_2811_2812)
        TD.state.setup.errors.add('WP89ERR')
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual('WP89', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000, 4)).decode)
        self.assertEqual(1, TD.state.vm.get_byte(TD.state.regs.R1 + TD.wa0pty))
...
class BeforeNameETAW(unittest.TestCase):
    ...
    def test_HFX_BA_SRT5(self):
        ...
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('SRT5', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000 + 4, 4)).decode)

    def test_HFX_AA_TKV_SRT5(self):
        TD.state.setup.aaa['WA0ET6'] = bytearray([TD.wa0hfx])
        TD.state.setup.aaa['WA0US3'] = bytearray([TD.wa0tkv])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual('SRT5', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000 + 4, 4)).decode)

    def test_ASC_ITN_fqtv_ETAS(self):
        TD.state.setup.aaa['WA0ASC'] = bytearray([0x01])
        TD.state.setup.aaa['WA0ET2'] = bytearray([TD.wa0itn])
        Pnr.add_fqtv(config.AAAPNR, TD.fqtv_gld)
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual('ETAS', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000 + 8, 4)).decode)

    def test_ASC_FTN_ETAS(self):
        TD.state.setup.aaa['WA0ASC'] = bytearray([0x01])
        TD.state.setup.aaa['WA0XX3'] = bytearray([TD.wa0ftn])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('ETAS', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000 + 8, 4)).decode)
    ...
    def test_FTD_ETK2(self):
        TD.state.setup.aaa['WA0XX3'] = bytearray([TD.wa0ftd])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual(13, TD.state.regs.R6)

    def test_AFU_subs_ETGN(self):
        TD.state.setup.aaa['WA0USE'] = bytearray([TD.wa0afu])
        Pnr.add_subs_card_seg(config.AAAPNR, ['TEST'])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertEqual('ETGN', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000 + 12, 4)).decode)

    def test_AFU_ETGN(self):
        TD.state.setup.aaa['WA0USE'] = bytearray([TD.wa0afu])
        label = TD.state.run('ETA5', aaa=True)
        self.assertEqual('$$ETAW$$.1', label)
        self.assertNotEqual('ETGN', DataType('X', bytes=TD.state.vm.get_bytes(TD.ebx000 + 12, 4)).decode)
