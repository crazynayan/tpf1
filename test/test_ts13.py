...
class SegmentTest(unittest.TestCase):
    ...
    def test_key_value(self):
        ...
        self.assertEqual('BASEREG', node.get_key_from_value('R1'))
        self.assertListEqual(['BASEREG', 'GET', 'INIT', 'FILE'], node.keys)
        self.assertListEqual([('BASEREG', 'R1'), ('GET', 'CORE'), ('INIT', 'YES'), ('FILE', 'NO')], node.operands)
        ...
        self.assertEqual('FILL', node.get_key_from_value('00'))
        self.assertListEqual(['D5', 'L4', 'FILL'], node.keys)
        self.assertListEqual([('D5', None), ('L4', None), ('FILL', '00')], node.operands)
        ...
        self.assertEqual('REG', node.get_key_from_value('R4'))
        self.assertListEqual(['ACTION', 'REG'], node.keys)
        self.assertEqual(2, len(node.operands))
        ...
        self.assertEqual('REG', node.get_key_from_value('R14'))
        self.assertSetEqual({'TS110020.2'}, node.next_labels)
        # GLOBZ REGR=R15
        node: RegisterData = self.seg.nodes['TS110020.2']
        ...
        # DETAC D8,CHECK=NO
        node: KeyValue = self.seg.nodes['TS110020.3']
        ...
        # RELCC D5
        self.assertListEqual(['D5'], self.seg.nodes['TS110040.2'].keys)
        # CRUSA S0=5,S1=E
        self.assertListEqual(['S0', 'S1'], self.seg.nodes['TS110040.3'].keys)
        self.assertEqual('E', self.seg.nodes['TS110040.3'].get_value('S1'))
        ...
        self.assertListEqual([('P1', 'R'), ('P2', '021014')], self.seg.nodes['TS110050.2'].operands)
        ...
        self.assertEqual("'MAXIMUM NUMBER OF NAMES PER PNR IS 99 - CREATE NEW PNR'",
                         self.seg.nodes['TS110060'].get_value('MSG'))
        ...
        self.assertEqual('SIZE', node.get_key_from_value('4096'))
        self.assertListEqual(['ALLOCATE', 'SREF', 'REG', 'SIZE', 'FILL', 'ERROR'], node.keys)
        self.assertEqual(6, len(node.operands))
        ...
        self.assertIsNone(self.seg.nodes['TS110060.2'].get_key_from_value('Invalid value'))
        # DBCLS REF=PD0_DF_REFX,FILE=PR001W
        self.assertEqual('PR001W', self.seg.nodes['TS110070.1'].get_value('FILE'))
        ...
        self.assertEqual(4, len(node.operands))
