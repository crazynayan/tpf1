...
from d21_backend.p1_utils.errors import UcdrError
...
def pars_to_date(days_from_start: int, full_year: bool = False) -> bytearray:
    if not 0 <= days_from_start <= 0x7FFF:
        raise UcdrError
    date = config.PARS_DAY_1 + timedelta(days=days_from_start)
    date_format: str = "%d%b%Y" if full_year else "%d%b"
    date_str = date.strftime(date_format).upper()
